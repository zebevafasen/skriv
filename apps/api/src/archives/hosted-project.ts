...
export async function loadHostedProjectArchive(
  ...
  const [project] = await context.db.select().from(projects).where(eq(projects.id, projectId)).limit(1);
  if (!project) return null;
  const actRows = await context.db.select().from(acts).where(eq(acts.projectId, projectId)).orderBy(asc(acts.position));
  const chapterRows = actRows.length
    ? await context.db.select().from(chapters).where(inArray(chapters.actId, actRows.map((row) => row.id))).orderBy(asc(chapters.position))
    : [];
  const sceneRows = chapterRows.length
    ? await context.db.select().from(scenes).where(inArray(scenes.chapterId, chapterRows.map((row) => row.id))).orderBy(asc(scenes.position))
    : [];
  const revisionRows = sceneRows.length
    ? await context.db.select().from(sceneRevisions).where(inArray(sceneRevisions.sceneId, sceneRows.map((row) => row.id))).orderBy(asc(sceneRevisions.createdAt))
    : [];
  const categoryRows = await context.db.select().from(compendiumCategories).where(eq(compendiumCategories.projectId, projectId)).orderBy(asc(compendiumCategories.position));
  const entryRows = await context.db.select().from(compendiumEntries).where(eq(compendiumEntries.projectId, projectId));
  const noteRows = await context.db.select().from(projectNotes).where(eq(projectNotes.projectId, projectId));
  const packRows = await context.db.select().from(projectIngredientPacks).where(eq(projectIngredientPacks.projectId, projectId));
  const threadRows = await context.db.select().from(chatThreads).where(eq(chatThreads.projectId, projectId));
  const messageRows = threadRows.length
    ? await context.db.select().from(chatMessages).where(inArray(chatMessages.threadId, threadRows.map((row) => row.id))).orderBy(asc(chatMessages.createdAt))
    : [];
  ...
  const archive = projectArchiveV5Schema.parse({
    ...
    manuscript: actRows.map((act) => ({
      ...
      chapters: chapterRows.filter((chapter) => chapter.actId === act.id).map((chapter) => ({
        id: chapter.id,
        actId: chapter.actId,
        title: chapter.title,
        position: chapter.position,
        scenes: sceneRows.filter((scene) => scene.chapterId === chapter.id).map((scene) => ({
          id: scene.id,
          ...
          revisions: revisionRows.filter((revision) => revision.sceneId === scene.id).map((revision) => ({
            id: revision.id,
    ...
    compendiumCategories: categoryRows.map((row) => ({ ...row, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) })),
    ...
    notes: noteRows.map((row) => ({ ...row, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) })),
    ...
    chatThreads: threadRows.map((thread) => ({
      ...
      messages: messageRows.filter((message) => message.threadId === thread.id).map((message) => ({
        ...message,
...
export async function importHostedProjectArchive(
  ...
  const categoryMap = new Map(archive.compendiumCategories.map((row) => [row.id, crypto.randomUUID()]));
  ...
  const messageMap = new Map(archive.chatThreads.flatMap((thread) => thread.messages.map((message) => [message.id, crypto.randomUUID()] as const)));
  const assetMap = new Map(assets.map((asset) => [asset.path, `data:${asset.mime};base64,${Buffer.from(asset.bytes).toString("base64")}`]));
  const assetFor = (kind: "cover" | "compendium", entryId?: string) => archive.assets.find((asset) => asset.target.kind === kind && (kind === "cover" || (asset.target.kind === "compendium" && asset.target.entryId === entryId)));
  ...
  const project = await context.db.transaction(async (tx) => {
    ...
    if (archive.compendiumCategories.length) await tx.insert(compendiumCategories).values(archive.compendiumCategories.map((row) => ({
    ...
    if (archive.projectIngredientPacks.length) await tx.insert(projectIngredientPacks).values(archive.projectIngredientPacks.map((row) => ({
      projectId,
    ...
    for (const act of archive.manuscript) {
      ...
      await tx.insert(acts).values({ id: actId, projectId, title: act.title, position: act.position });
      for (const chapter of act.chapters) {
        const chapterId = chapterMap.get(chapter.id) as string;
        await tx.insert(chapters).values({ id: chapterId, actId, title: chapter.title, position: chapter.position });
        for (const scene of chapter.scenes) {
          ...
          await tx.insert(scenes).values({
            ...
            metadata: sceneMetadataSchema.parse({
              ...
              presentCharacterEntryIds: scene.metadata.presentCharacterEntryIds.flatMap((id) => entryMap.get(id) ?? []),
            }),
          });
          if (scene.revisions.length) await tx.insert(sceneRevisions).values(scene.revisions.map((revision) => ({
    ...
    if (archive.compendium.length) await tx.insert(compendiumEntries).values(archive.compendium.map((entry) => {
      const image = assetFor("compendium", entry.id);
      return {
        id: entryMap.get(entry.id) as string,
        projectId,
        name: entry.name,
        typeId: entry.typeId.startsWith("custom.") ? `custom.${categoryMap.get(entry.typeId.slice(7)) ?? entry.typeId.slice(7)}` : entry.typeId,
    ...
    if (archive.notes.length) await tx.insert(projectNotes).values(archive.notes.map((note) => ({
      id: crypto.randomUUID(), projectId, title: note.title, document: note.document, plainText: note.plainText, pinned: note.pinned, version: note.version,
    })));
    for (const thread of archive.chatThreads) {
      ...
      for (const source of thread.contextSources) {
        if (source.kind === "act") { const id = actMap.get(source.id); if (id) contextSources.push({ ...source, id }); }
        else if (source.kind === "chapter") { const id = chapterMap.get(source.id); if (id) contextSources.push({ ...source, id }); }
        else if (source.kind === "scene") { const id = sceneMap.get(source.id); if (id) contextSources.push({ ...source, id }); }
        else if (source.kind === "compendium_entry") { const id = entryMap.get(source.id); if (id) contextSources.push({ ...source, id }); }
        else if (source.kind === "compendium_type" && source.typeId.startsWith("custom.")) contextSources.push({ ...source, typeId: `custom.${categoryMap.get(source.typeId.slice(7)) ?? source.typeId.slice(7)}` });
      ...
      if (thread.messages.length) await tx.insert(chatMessages).values(thread.messages.map((message) => ({
