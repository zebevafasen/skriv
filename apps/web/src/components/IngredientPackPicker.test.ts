...
import { ingredientPackMatchesSearch, toggleIngredientPackSelection } from "./IngredientPackPicker.js";
...
describe("hierarchical ingredient pack selection", () => {
  it("adds a partial collection and removes a fully selected collection", () => {
    ...
    expect(toggleIngredientPackSelection(new Set(["one", "two", "outside"]), ["one", "two"])).toEqual([
      "outside",
    ]);
  });
  ...
  it("searches pack metadata, collection names, and definition labels", () => {
    ...
    expect(ingredientPackMatchesSearch(pack, "Fantasy", "science fiction", definitions)).toBe(false);
  });
});
