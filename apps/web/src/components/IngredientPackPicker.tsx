import type { ProjectIngredientPack, IngredientPack, IngredientPackCatalog } from "@asterism/contracts";
                ...
                <div className="ingredient-category-contents">
                  <div className="ingredient-collection-list">
                    {collections.map(({ collection, allPacks: collectionPacks, packs }) => {
                      ...
                      const fullySelected =
                        packIds.length > 0 && selectedCount === packIds.length;
        ...
        <section className="archived-ingredient-packs">
          ...
          <div className="ingredient-pack-grid">
            {archivedIngredientPacks.map((pack) => (
              ...
              >
                <span className="ingredient-pack-check"><Check size={13} /></span>
                <span className="archived-ingredient-pack-copy"><strong>{pack.name}</strong><small>{pack.description}</small></span>
              </button>
            ))}
          </div>
        </section>
...
function IngredientPackCard({
  ...
  return (
    <article className={`ingredient-pack-card${selected ? " selected" : ""}`}>
      <button type="button" className="ingredient-pack-select" disabled={disabled} onClick={onToggle}>
      ...
      <button type="button" className="ingredient-pack-preview-toggle" onClick={() => setPreview(!preview)}>
