...
export function PromptsPage() {
  ...
  const copy = useMutation({
    mutationFn: (id: string) => asterism().prompts.copy(id),
  ...
  const bind = useMutation({
    mutationFn: (prompt: PromptDefinition | null) => {
      const workflow = prompt?.workflow ?? draft?.workflow;
      if (!workflow) throw new Error("Select a prompt workflow first.");
      return asterism().prompts.bind(workflow, prompt?.id ?? null);
    },
    onSuccess: () => client.invalidateQueries({ queryKey: ["prompts"] }),
  });
